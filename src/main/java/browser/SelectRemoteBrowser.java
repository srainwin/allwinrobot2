...
public class SelectRemoteBrowser {
	...
	/**
	 * 获取selenium grid分发运行的node服务器IP，用于sikuli vnc指定远程ip控制，保证selenium grid和sikuli vnc操作同一台机器
	 */
	public String getGridIP(String huburl) {
		...
		try {
			...
			// 根据RemoteWebDriver获取的session传到hub分发节点的url上发送请求，然后解析请求回应可得ip信息
			...
			logger.info("解析hub分发节点详情的请求回应后得知选用的node节点是：" + url.getAsString());
