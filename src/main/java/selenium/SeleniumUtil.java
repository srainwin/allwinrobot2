...
public class SeleniumUtil {
	...
	public void launchBrowser(String browserName, String driverConfigFilePath, String isRemote, String huburl, int timeOut) {
		if( isRemote.equals("true") ){
			...
			threadWebDriver.set(selectremotebrowser.selectByName(browserName, huburl));
			...
			// 并由ThreadLocal提供线程局部变量
			threadWebDriver.set(selectlocalbrowser.selectByName(browserName, driverConfigFilePath));
			// 然后在每个[@test用例中]或者[@test用例中调用的封装方法里]使用threadLocalWebDriver.get()来获取单个线程绑定的driver
		}else{
			logger.warn("testng.xml的isRemote参数值填写有误，请输入true或false!");
			Assert.fail("testng.xml的isRemote参数值填写有误，请输入true或false!");
		}
...
//			fullWindow();
			...
			Assert.fail("启动" + browserName + "浏览器发生异常");
		}
	}
	...
	public void fullWindow() {
		try {
			threadWebDriver.get().manage().window().fullscreen();
			logger.info("成功全屏浏览器");
		} catch (Exception e) {
	...
	public void get(String testurl) {
		try {
			...
			threadWebDriver.get().get(testurl);
	...
	public void forward() {
		try {
			threadWebDriver.get().navigate().forward();
			logger.info("成功重返网页");
		} catch (Exception e) {
	...
	public void cookiesSaveInFile(String cookiesConfigFilePath) {
		...
		pause(2000);
		Set<Cookie> cookies = threadWebDriver.get().manage().getCookies();
		...
		try {
			...
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("获取网站cookies并写入到cookies文件中发生异常");
        }
	...
	public void addcookies(String cookiesConfigFilePath){
		try {
            ...
            while ((line = br.readLine()) != null){
                ...
                threadWebDriver.get().manage().addCookie(ck);
            }
            br.close();
	...
	public WebElement findElementBy(By byElement) {
		...
		try {
			element = threadWebDriver.get().findElement(byElement);
			//高亮显示
			highLight(element);
			logger.info("成功定位元素");
		} catch (Exception e) {
	...
	public List<WebElement> findElementsBy(By byElement) {
		...
		try {
			elements = threadWebDriver.get().findElements(byElement);
	...
	public WebElement findElementByWait(int timeOutInSeconds, final By byElement) {
		...
		try {
			//元素最多等待timeOut秒
			WebDriverWait webDriverWait = new WebDriverWait(threadWebDriver.get(), timeOutInSeconds);
...
//			});
			...
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("超时!! " + timeOutInSeconds + " 秒之后还没找到元素 [" + byElement + "]");
		} catch (Exception e) {
			logger.error("给定的时间内定位查找元素异常", e);
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("给定的时间内定位查找元素异常");
		}
		return element;
	}
	...
	public void implicitlyWait(long timeOut) {
		try {
			threadWebDriver.get().manage().timeouts().implicitlyWait(timeOut, TimeUnit.SECONDS);
	...
	public void pageLoadTimeout(long pageLoadTime) {
		try {
			threadWebDriver.get().manage().timeouts().pageLoadTimeout(pageLoadTime, TimeUnit.SECONDS);
			logger.info("成功加载页面");
		} catch (Exception e) {
			logger.error("超时未能完全加载页面", e);
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("超时未能完全加载页面");
		}
	}
	...
	public void setScriptTimeout(long timeOut) {
		try {
			threadWebDriver.get().manage().timeouts().setScriptTimeout(timeOut, TimeUnit.SECONDS);
			logger.info("成功异步执行脚本");
		} catch (Exception e) {
			logger.error("超时未能异步执行脚本", e);
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("超时未能异步执行脚本");
		}
	}
	...
	public String getTitle() {
		String title = null;
		try {
			title = threadWebDriver.get().getTitle();
			logger.info("成功获取页面标题");
		} catch (Exception e) {
			logger.error("获取页面标题异常", e);
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("获取页面标题异常");
		}
		return title;
	}
	...
	public void clear(By byElement) {
		try {
			...
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("清除元素 [" + byElement + "] 上的内容异常");
		}
	}
	...
	public void click(By byElement) {
		try {
			...
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("你点击的元素:[" + byElement + "]不再存在!");
		} catch (Exception e) {
			logger.error("点击元素 [" + byElement + "]发生异常", e);
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("点击元素 [" + byElement + "]发生异常");
		}
	}
	...
	public void type(By byElement, CharSequence sendkeys) {
		try {
			...
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("输入 [" + sendkeys + "] 到 元素[" + byElement + "]异常");
		}
	}
	...
	public void pressKeys(By byElement, Keys key, String keyword) {
		try {
			...
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("键盘操作" + key.name() + "+" + keyword + "异常");
		}
	}
	...
	public Alert switchToAlert(long waitMillisecondsForAlert)  {
		...
		for (long i = 0; i < waitMillisecondsForAlert; i += ONE_ROUND_WAIT) {
			try {
				alert = threadWebDriver.get().switchTo().alert();
				return alert;
	...
	public void inFrame(String nameOrId) {
		try{
			threadWebDriver.get().switchTo().frame(nameOrId);
			logger.info("成功切换frame");
		}catch(Exception e){
			logger.error("切换frame发生异常",e);
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("切换frame发生异常");
		}
	}
	...
	public void inFrame(int index) {
		try{
			threadWebDriver.get().switchTo().frame(index);
			logger.info("成功切换frame");
		}catch(Exception e){
			logger.error("切换frame发生异常",e);
			//由testng的失败断言来控制用例运行是否失败
	...
	public void inFrame(By ByframeElement) {
		try {
			...
			threadWebDriver.get().switchTo().frame(findElementByWait(5,ByframeElement));
			logger.info("成功切换frame");
		} catch (Exception e) {
			logger.error("切换frame发生异常",e);
			//由testng的失败断言来控制用例运行是否失败
	...
	public String getHandleTitle (){
		String title = null;
		try{
			threadWebDriver.get().getTitle();
			logger.info("成功获取窗口标题");
		}catch(Exception e){
			logger.error("获取窗口标题发生异常",e);
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("获取窗口标题发生异常");
		}
		return title;
	}
	...
	public String getCurrentHandle() {
		String handle = null;
		try {
			handle = threadWebDriver.get().getWindowHandle();
			logger.info("成功获取所有窗口句柄");
		} catch (Exception e) {
			logger.error("获取所有窗口句柄发生异常",e);
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("获取所有窗口句柄发生异常");
		}
		return handle;
	}
	...
	public Set<String> getAllHandles() {
		...
		try {
			handles = threadWebDriver.get().getWindowHandles();
			logger.info("成功获取所有窗口句柄");
		} catch (Exception e) {
			logger.error("获取所有窗口句柄发生异常",e);
			//由testng的失败断言来控制用例运行是否失败
	...
	public void switchToHandle(String handle) {
		try {
			...
			threadWebDriver.get().switchTo().window(handle);
			logger.info("成功切换handle");
		} catch (Exception e) {
			logger.error("切换handle发生异常",e);
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("切换handle发生异常");
		}
	}
	...
	public void mouseMoveToElement(By byElement,int xOffset,int yOffset) {
		try{
			...
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("移动鼠标到指定元素的(x,y)位置发生异常");
		}
	}
	...
	public void mouseDragAndDrop(By BySourceElement,By ByTargetElement) {
		try{
			...
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("鼠标拖拽source元素到target元素位置发生异常");
		}
	}
	...
	public void mouseDragAndDrop(By BySourceElement,int xOffset,int yOffset) {
		try{
			...
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("鼠标拖拽source元素到(xOffset, yOffset)位置发生异常");
		}
	}
	...
	public boolean isSelected(By byElement) {
		...
		try{
			...
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("检查CheckBox是否勾选发生异常");
		}
		return flag;
	}
	...
	public void selectByIndex(By byElement, int index) {
		try{
			...
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("根据index角标选择下拉选项发生异常");
		}
	...
	public List<WebElement> getSelectOptions(By byElement) {
		...
		try{
			...
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("获得当前下拉的所有选项的元素定位发生异常");
		}
		return optionsList;
	}
	...
	public String getInputValue(String nameOrId, String nameOrIdValue) {
		...
		try{
			...
			case "name":
				...
				value = (String) ((JavascriptExecutor) threadWebDriver.get()).executeScript(jsByName);
				break;
			case "id":
				// 把JS执行的值返回出去
	...
	public int getTableColCount(By byTableElement, int rownum) {
		...
		try{
			...
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("获取表格指定行的列数/字段数发生异常");
		}
	...
	public Object executeJS(String js) {
		Object obj = null;
		try{
			obj = ((JavascriptExecutor) threadWebDriver.get()).executeScript(js);
			logger.info("成功执行JavaScript语句：[" + js + "]");
		}catch(Exception e){
			logger.error("执行JavaScript语句：[" + js + "]发生异常",e);
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("执行JavaScript语句：[" + js + "]发生异常");
		}
		return obj;
	}
	...
	public Object executeJS(String js, Object... args) {
		Object obj = null;
		try{
			((JavascriptExecutor) threadWebDriver.get()).executeScript(js, args);
			logger.info("成功执行JavaScript语句[" + js + "]");
		}catch(Exception e){
			logger.error("执行JavaScript语句[" + js + "]异常",e);
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("执行JavaScript语句[" + js + "]异常");
		}
		return obj;
	}
	...
	public void isActualContainsExpect(String actual, String expect) {
		try {
			...
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("实际文本[" + actual + "]不包含期望文本[" + expect + "]");
		}
	}
	...
	public void assertTrue(By byElement, String textcontent) {
		try{
			...
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("断言获取元素的text是否含有指定内容发生异常");
		}
	...
	public void loginOnWinGUI(String username, String password, String url) {
		try{
			threadWebDriver.get().get(username + ":" + password + "@" + url);
			logger.info("成功处理windows GUI的登陆弹出框");
		}catch(Exception e){
			logger.error("处理windows GUI的登陆弹出框发生异常",e);
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("处理windows GUI的登陆弹出框发生异常");
		}
	}
	...
	public void uploadFile(By byElement, File file) {
		...
		try {
			...
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("通过定位上传按钮后sendKeys()上传文件发生异常");
		}
	}
	...
	public void autoitExe(String exeName, String autoitFolderPath){
		...
		try{
			...
			if( staus == 0){
				...
				//由testng的失败断言来控制用例运行是否失败
				Assert.fail("执行autoit脚本发生异常，异常终止脚本进程");
			}
		}catch(Exception e){
			logger.error("执行autoit的exe脚本发生异常",e);
			//由testng的失败断言来控制用例运行是否失败
			Assert.fail("执行autoit的exe脚本发生异常");
		}
	}
